...
.controller( 'ReferenzenCtrl', function AppCtrl ( $scope, $http, $timeout ) {
	...
	$scope.switch = function(activeRef) {
		...
		jQuery('.doors .right-door .inner').fadeOut(500, function() {
			...
			$timeout(function() {
			    ...
			    jQuery('.doors .left-door .inner').css({'background-image': 'url(portfolio/'+$scope.activeRef.image+'/'+$scope.activeRef.image+'.jpg)'});
	...
	$scope.switchInner = function(i) {
		...
		jQuery('.doors .left-door .inner').fadeOut(400, function() {
			...
			$timeout(function() {
			    $scope.$apply();
			    jQuery('.doors .left-door .inner').css({'background-image': 'url(portfolio/'+$scope.activeRef.image+'/'+img+'.jpg)'});
	...
	$scope.preload = function(id) {
		...
		var img  = new Image();
	    img.src = 'portfolio/'+activeRef.image+'/'+activeRef.image+'.jpg';
	};
